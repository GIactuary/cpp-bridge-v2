...
function renderResults(data) {
    ...
    // Store result for funnel handoff and show quiz button
...
function goToQuiz() {
    // Gather all calculator inputs and outputs
    const gender = document.querySelector('input[name="gender"]:checked').value;
    const calculatorData = {
        // User inputs
        gender: gender,
        current_age: parseInt($('age').value),
        cpp_estimate_at_65: parseFloat($('cpp').value) || 1000,
        rrsp_savings: parseFloat($('savings').value) || 0,
        health_status: $('health').value,
        real_rate_of_return: parseFloat($('ror').value) || 0.035,
        wage_growth: parseFloat($('wage').value) || 0.011,
